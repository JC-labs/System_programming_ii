...
struct recrd emptyElm = {{"",0},		// ключові поля			// key fields
						{FunctionalField::no_value},0};	// функціональні поля	// functional fields
//------------------------------------------------------
...
struct recrd* selNmb(struct recrd*tb, unsigned int nElm) {
	...
	_asm {
		sub eax, eax
		mov eax, nElm  //Valid if only 'al' is a part of 'eax'
...
struct recrd* updNmb(struct recrd*pElm, struct recrd*tb, unsigned int nElm, int *pQnElm) {
	unsigned int n = *pQnElm;
	if (nElm >= n)return 0;
	tb[nElm] = *pElm;
	return &tb[nElm];
...
int cmpKey(struct recrd* el, struct keyStr kArg) {
	int i = cmpStr((unsigned int*) el->key.str,
		(unsigned int*) kArg.str);
	if (i)return i;
...
int cmpKys(struct keyStr *k0, struct keyStr *kArg) {
	int i = cmpStr((unsigned int*) k0->str,
		(unsigned int*) kArg->str);
	if (i)return i;
...
struct recrd*insBin(struct recrd*pElm, struct recrd*tb, int*pQnElm) {
...
struct recrd*delBin(struct recrd*pElm, struct recrd*tb, int*pQnElm) {
	struct recrd*pr = selBin(pElm->key, tb, *pQnElm);
	// при знаходженні елементу помітити його для вилучення 
	// if appropriate element is found to mark it as deletedif(pr)
...
struct recrd*updBin(struct keyStr, struct recrd*pElm, struct recrd*tb, int*pQnElm) {
	struct recrd*pr = selBin(pElm->key, tb, *pQnElm);
